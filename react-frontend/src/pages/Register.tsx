...
export default function Register() {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        setErrors([]);
        register({ name: name, email: email, password: password, password_confirmation: confirmedPassword }).then(res => {
            if (res.data.errors) {
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bld text-gray-800">Register</h2>
      {errors.length > 0 && (
